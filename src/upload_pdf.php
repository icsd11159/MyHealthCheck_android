...
//$file=$_POST['$file'] ;
		...
		$dta=addslashes(file_get_contents($_FILES['file']['tmp_name']));
...
$id_e = mysqli_real_escape_string($link,$id_e);
  $sql="UPDATE upload_pdf SET name='$file',type='$file_type',size='$file_size',content='$dta' WHERE id_exam='$id_e'";
  ...
  if ($result) {
        mysqli_commit($link);
		  ...
		  echo "Ανέβηκε το pdf με όνομα: ".$file; 
		 // setPdf(string $pdf) : Pdf
        } else {
